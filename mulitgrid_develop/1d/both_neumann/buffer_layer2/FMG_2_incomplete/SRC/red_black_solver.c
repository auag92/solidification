void rb_solve(double *y,double *f,int begin,double *error,int mesh_tracker)
{
	...
	for(i=begin+start[mesh_tracker];i<=end[mesh_tracker]-1;i+=2)//leaving out the buffer points  
	{

		if((i-start[mesh_tracker])==1)
		{
			y_star=y[i+1]-dx*dx*f[i];
		}	

		else if ((i-start[mesh_tracker])==nodes_x[mesh_tracker]-2)
		{
			y_star=y[i-1]-dx*dx*f[i];
		}		
